...
var db;
var s;
MongoClient.connect('mongodb://localhost:27017/FashionWear',function(err,database)
{
    if(err) return console.log(err)
    db=database.db('FashionWear')
    app.listen(5000,() => {
...
app.post('/update',(req,res)=>{
    ...
    app.post('/delete', (req,res)=>{
      ...
      app.post("/edit1",(req,res)=>{
        db.collection("ladies").findOneAndUpdate({pid:req.body.id},{$set:{sp:req.body.pcost}},{sort :{ _id:-1}},(err,result)=>{
            //console.log(result);
            if(err)
            return console.log(err);
            res.redirect("/"); 
